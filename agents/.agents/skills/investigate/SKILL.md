...
This phase ends at the handoff. Preserve the working record and remaining uncertainties. OPERATIONS.md lists the repository's existing runs before any allocation, so a new investigation into an already-investigated repository surfaces them rather than quietly starting beside them. When the user resumes one, read its brief and manifest, establish whether they want to reframe or resume the partner conversation, and reuse the recorded paths rather than allocating another run.
